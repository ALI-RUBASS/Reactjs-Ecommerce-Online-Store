...
const AdminSellerReports = () => {
    ...
    <div style="padding: 20px;">
        ${users2.length !== 0 ? `
            ...
            <p style="border: 1px solid black; padding: 8px;"><b>Total Orders: </b>${users2.length}</p>
...
`;
    ...
    function GenerateSellReport(sellerid)
    {
        fetch('http://localhost:5000/query', {
            method: 'POST',
            headers: {
                ...
                <table class="${classes.table}" style="border-collapse: collapse;">
                    <thead>
                        <tr>
                            ...
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Brand</b></th>
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Category</b></th>
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Vendor</b></th>
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Rate</b></th>
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Quantity</b></th>
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Price</b></th>
                            <th style="border: 1px solid black; padding: 8px;"><b class="text-black">Timestamp</b></th>
    ...
    function GeneratePurchaseReport(sellerid)
    {
        fetch('http://localhost:5000/query', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                query: "Select o.quantity, p.productid, p.vendorid, p.name, p.description, p.brand, p.category, p.quantity, p.price, p.image,o.orderID,o.vendorid,v.name,o.purchasedate from sellerorders o join stock p on o.vendorid=p.vendorid AND o.productid = p.productid AND o.sellerid="+sellerid+" Join vendor v on o.vendorid = v.vendorid"
            })
        })
            .then(res => res.text())
            .then(data => {
    ...
    return (
        <div>
            ...
            <h1 className="text-lg ml-3 text-slate-500 font-bold mb-4">Seller Reports</h1>
                            ...
                            {users.map((user) => (
                                <TableRow key={user[0]}>
                                    ...
                                    <TableCell>
                                        <TableCell>
                                            <Button
                                                onClick={() => GenerateSellReport(user[0])}
                                                variant="contained" color="primary">Sell</Button>
                                        </TableCell>
                                        <TableCell>
                                            <Button
                                                onClick={() => GeneratePurchaseReport(user[0])}
                                                variant="contained" color="primary">Purchase</Button>
                                        </TableCell>
                                    </TableCell>
                                </TableRow>
                            ))}
